...
function SelectRow({ options, value, onChange }: { options: string[]; value: string; onChange: (v: string) => void }) {
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }} contentContainerStyle={{ gap: 6 }}>
      {options.map(o => (
        <TouchableOpacity key={o} onPress={() => onChange(o)} style={[styles.selChip, value === o && styles.selChipOn]}>
          <Text style={[styles.selText, value === o && styles.selTextOn]}>{o}</Text>
        </TouchableOpacity>
      ))}
    </ScrollView>
  );
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
      <View style={styles.container}>
        ...
        <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <FieldLabel text="Nombre del ejercicio" />
          ...
          <FieldLabel text="Descripción / Técnica" />
          ...
          <FieldLabel text="Pausa entre series" />
          <SelectRow options={RESTS.map(r => r.label)} value={RESTS.find(r => r.val === rest)?.label || '60 seg'} onChange={v => setRest(RESTS.find(r => r.label === v)?.val || 60)} />

          <FieldLabel text="Grupo muscular" />
          <SelectRow options={MUSCLES} value={muscle} onChange={v => setMuscle(v as MuscleGroup)} />

          <FieldLabel text="Equipo" />
          <SelectRow options={EQUIPS} value={equip} onChange={setEquip} />

          <FieldLabel text="Dificultad" />
          <SelectRow options={DIFFS} value={diff} onChange={v => setDiff(v as Exercise['diff'])} />
          ...
          <FieldLabel text="Video de YouTube" />
          ...
          <FieldLabel text="Imagen de referencia" />
